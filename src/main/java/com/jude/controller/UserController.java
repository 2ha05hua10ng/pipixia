...
import com.jude.service.RoleService;
...
public class UserController {
	 ...
	 * @param user
	 * @return
	 * @Valid  校验 实体中的属性加的约束是否正确
	 *BindingResult 内部通过集成hiberbate-validation  已经实现了JSR-349验证规范几口，可以直接使用
	 * 一般用在COntroller中验证前端传来的参数
	 * imagecode   前端传过来的验证码
	 */
	@ResponseBody
	@PostMapping("/login")
	public Map<String,Object> login(String imageCode, @Valid User user, BindingResult bindingResult, HttpSession session){
		...
		Subject subject= SecurityUtils.getSubject();  //shiro权限
		//创建一个用户
		UsernamePasswordToken token=new UsernamePasswordToken(user.getUserName(), user.getPassword());
		try{
			//是否记住用户
			//token.setRememberMe(true);
			subject.login(token); // 登录认证      进入权限认证
